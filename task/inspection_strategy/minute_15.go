...
func Strategy15Min() error {
	logs.Info("【15分钟】级别的定时任务启动执行...")
	...
	for _, business := range businesses {
		...
		for _, service := range serviceMongos {
			PerformInspection(businessId, service.Id, msgChannel, restrainMsgChannel, inspection.ONE_QUARTER_CODE)
			logs.Info("15分钟巡检执行完毕，如果监测到问题开始发送叮叮--------")
		}

	}
	// dingMsg中的「线上巡检」为消息关键字，不可变更
	dingMsg := "小钻风【15分钟】级别线上巡检发现异样, 快去排查一下吧。\n"
	...
	if len(msgList) > 0 {
		...
		if IS_OPEN_SENDDING_MSG {
			logs.Info("开始发送叮叮消息，【巡检次数15分钟/次】")
			DingSend(dingMsg)
		}
	} else {
		logs.Info("本次执行无异常，无需发送叮叮")
	}
	// dingMsg中的「线上巡检」为消息关键字，不可变更
