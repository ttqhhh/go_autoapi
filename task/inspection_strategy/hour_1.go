...
func Strategy1Hour() error {
	logs.Info("【小时】级别的定时任务启动执行...")
	...
	for _, business := range businesses {
		...
		for _, service := range serviceMongos {
			PerformInspection(businessId, service.Id, msgChannel, inspection.ONE_HOUR_CODE)
		}

	}
	// dingMsg中的「线上巡检」为消息关键字，不可变更
	dingMsg := "小钻风【小时】级别线上巡检发现异样, 快去排查一下吧。\n"
