...
<script>
    ...
    layui.use('table', function () {
        ...
        $("#add").on("click", function () {
            var title = "新增";
            var url = "/service/addIndex";
            layer.open({
                title: title,
                type: 2,
                // content: $('#service') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
                content: url,
                area: ['800px', '600px']
            });
        });
        window.edit = function (id) {
            var title = "编辑";
            var url = "/service/addIndex?id=" + id;
            layer.open({
                title: title,
                type: 2,
                // content: $('#service') //这里content是一个DOM，注意：最好该元素要存放在body最外层，否则可能被其它的相对元素所影响
     ...
     *  初始化列表数据
     */
        // 定一个业务线代码与名称映射关系   0：最右，1：皮皮，2：海外，3：中东，4：妈妈
    var businessMap = new Map();
    // businessMap.set(0, "最右");
    // businessMap.set(1, "皮皮");
    // businessMap.set(2, "海外");
    ...
    function tableInit(business, service_name, pageNo, pageSize, action) {
        ...
        $.ajax({
            url: "/service/page?business=" + business + "&service_name=" + service_name + "&page_no=" + pageNo + "&page_size=" + pageSize,
            type: "GET",
            dataType: "json",
            success: function (data) {
                ...
                $.each(tableData, function (index, val) {
                    var tr = "<tr>" +
                        ...
                        "<td>" + businessMap.get(val.business) + "</td>" +
                        "<td>" + "admin" + "</td>" +
                        "<td>" +
                        // "<button type='button' class='layui-btn layui-btn-normal' onclick='see("+val.id+")'>查看</button>" +
                        "<button type='button' class='layui-btn layui-btn-normal' onclick='edit(" + val.id + ")'>编辑</button>" +
                        "<button type='button' class='layui-btn layui-btn-normal' onclick='del(" + val.id + ")'>删除</button>" +
                        "</td>" +
                        "</tr>";
                    $("tbody").append(tr);
                })
    ...
    function list() {
        ...
        tableInit(business, service_name, null, null)
    }
    ...
    function prevPage() {
        ...
        tableInit(business, service_name, currentPage - 1, null, "prev");
    }
    ...
    function flip(direction) {
        ...
        var service_name = $("#service_name").val()
        // 确定当前页面
        var currentPage = $("#currrentPage").val();
        var pageNo;
        if ("prev" == direction) {
        ...
        tableInit(business, service_name, pageNo, null, "next");
    }
</script>
...
{{/*                <option value="5">妈妈社区</option>*/}}
        ...
        <div style="float: left; margin-left: 50px">
            <label>服务名：</label>
            <input id="service_name" class="layui-select" type="text" placeholder="请输入服务名">
        </div>
    ...
    <div>
        <div style="float: right; margin-right: 25px">
            <button type="button" id="next" class="layui-btn layui-btn-normal" onclick="flip('next')">下一页</button>
        </div>
        <div style="float: right; margin-right: 25px">
            第&nbsp;&nbsp; <input id="currrentPage" value="1" class="layui-select" type="text" disabled> &nbsp;&nbsp;页
        </div>
        <div style="float: right; margin-right: 25px">
            <button type="button" id="prev" class="layui-btn layui-btn-normal" onclick="flip('prev')">上一页</button>
        </div>
        <div style="float: right; margin-right: 25px">
            共&nbsp;&nbsp; <input id="total" class="layui-select" type="text" disabled> &nbsp;&nbsp;条
        </div>
    </div>
</div>
</body>
